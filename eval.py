...
from model.stan_model import StanMindConfig, StanMindForCausalLM
...
def init_model(args):
    ...
    print(
        f"StanMind模型参数: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)"  # ！修正：原残留MiniMind命名
    )
