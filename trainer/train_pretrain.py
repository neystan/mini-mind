...
from model.stan_model import StanMindConfig
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start = start_step + 1):
        ...
        lr = get_lr(epoch * iters + step, args.epochs * iters, args.learning_rate)  # 动态学习率计算
        ...
        with autocast_ctx:
            ...
            # 计算损失
            loss = res.loss
            loss = loss / args.accumulation_steps  # 梯度累积，平均损失

            # 反向传播
            # 梯度下降
        scaler.scale(loss).backward()   #梯度缩放 与 反向传播
        if (step + 1) % args.accumulation_steps == 0:
            scaler.unscale_(optimizer)  # 在梯度裁剪前取消缩放
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)  # 梯度裁剪
            scaler.step(optimizer)  # 优化器更新参数
            scaler.update()  # 更新缩放器
            optimizer.zero_grad(set_to_none=True)  # 清空梯度，释放内存
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="StanMind Pretraining")
